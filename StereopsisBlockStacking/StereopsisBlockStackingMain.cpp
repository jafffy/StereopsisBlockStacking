...
void StereopsisBlockStackingMain::SetHolographicSpace(HolographicSpace^ holographicSpace)
{
    ...
    m_spinningCubeRenderer = std::make_unique<SpinningCubeRenderer>();
...
HolographicFrame^ StereopsisBlockStackingMain::Update()
{
	...
	if (pointerState != nullptr) {
		...
		DirectX::BoundingBox bb = m_spinningCubeRenderer->GetBoundingBox();
		float distance = 0.0f;
		...
		if (bb.Intersects(
			...
			distance)) {
			m_spinningCubeRenderer->isGrabbed = true;
		}
		else {
			m_spinningCubeRenderer->isGrabbed = false;
		}

		if (m_spinningCubeRenderer->isGrabbed) {
			auto diff = m_spinningCubeRenderer->GetPosition() - pose->Head->Position;
			float distance = 0.0f;

			DirectX::XMStoreFloat(&distance, DirectX::XMVector3Length(DirectX::XMLoadFloat3(&diff)));

			m_spinningCubeRenderer->SetPosition(pose->Head->Position + pose->Head->ForwardDirection * distance);
		}
	}
